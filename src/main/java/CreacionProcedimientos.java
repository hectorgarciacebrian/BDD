...
public class CreacionProcedimientos{
    ...
    public static void main(String[] args) {
        ...
        String[] procedures = {
            ...
            BEGIN
                -- Si es director, desvincularlo de la sucursal (poner a NULL)
            ...
            BEGIN
                -- El Trigger trg_Salario_NoDisminuye se encargara de verificar si baja
            ...
            CREATE OR REPLACE PROCEDURE pr_Trasladar_Empleado (
                ...
                p_nueva_dir IN VARCHAR2 DEFAULT NULL -- Opcional
            ) IS
            BEGIN
                -- Si p_nueva_dir es NULL, la columna se pondrá a NULL (correcto segun enunciado)
                UPDATE Empleado 
            ...
            CREATE OR REPLACE PROCEDURE pr_Alta_Sucursal (
                ...
                p_director   IN NUMBER DEFAULT NULL -- Opcional
            ) IS
            BEGIN
            ...
            // 8. Gestionar Suministro (Alta/Actualizacion + Stock)
            """
            CREATE OR REPLACE PROCEDURE pr_Gestionar_Suministro (
                ...
                v_existe NUMBER;
            BEGIN
                -- 1. Verificar existencia del registro en Pide
                ...
                -- 3. Actualizar Stock del Vino (Nota 2)
                -- Restamos la cantidad pedida. Si es devolucion (negativo), -- = +
                UPDATE Vino SET stock = stock - p_cantidad WHERE cod_vino = p_cod_vino;
                
                -- Verificacion simple de integridad referencial si vino no existe
                IF SQL%ROWCOUNT = 0 THEN
                     RAISE_APPLICATION_ERROR(-20012, 'Error: El vino indicado no existe.');
                END IF;
            END;
            """,

            // 9. Dar de baja suministros (Restaurar Stock + Borrar)
            """
            CREATE OR REPLACE PROCEDURE pr_Baja_Suministros (
                p_cod_c    IN NUMBER,
                p_cod_suc  IN NUMBER,
                p_cod_vino IN NUMBER,
                p_fecha    IN DATE DEFAULT NULL -- Opcional
            ) IS
                v_cantidad_restaurar NUMBER := 0;
            BEGIN
                -- 1. Calcular cuanto stock hay que devolver a la estanteria
                IF p_fecha IS NOT NULL THEN
                    -- Caso fecha especifica
                    ...
                    WHERE cod_cliente = p_cod_c AND cod_sucursal = p_cod_suc 
                      AND cod_vino = p_cod_vino AND fecha_pide = p_fecha;
                ELSE
                    -- Caso borrar TODOS los suministros de ese vino
                    SELECT NVL(SUM(cantidad),0) INTO v_cantidad_restaurar
                    FROM Pide
                    WHERE cod_cliente = p_cod_c AND cod_sucursal = p_cod_suc 
                ...
                -- 2. Restaurar Stock
                UPDATE Vino SET stock = stock + v_cantidad_restaurar WHERE cod_vino = p_cod_vino;
            END;
            """,

            // 10. Dar de alta pedido sucursal (Solicita + Stock)
            """
            ...
            BEGIN
                ...
                IF SQL%ROWCOUNT = 0 THEN
                    RAISE_APPLICATION_ERROR(-20014, 'Error: Vino no encontrado para actualizar stock.');
                END IF;
            END;
            """,
            
            // 11. Dar de baja pedidos de una sucursal (Solicita)
            """
            CREATE OR REPLACE PROCEDURE pr_Baja_Pedido_Suc (
                ...
                v_cantidad_restaurar NUMBER := 0;
            BEGIN
                -- 1. Calcular cantidad a devolver al stock
                IF p_fecha IS NOT NULL THEN
                    -- Caso fecha específica
                    SELECT NVL(SUM(cantidad),0) INTO v_cantidad_restaurar
                    FROM Solicita
                    ...
                    WHERE cod_sucursal = p_cod_suc_pide AND cod_sucursal_prov = p_cod_suc_prov 
                      AND cod_tipo_vino = p_cod_vino AND fecha_sol = p_fecha;
                ELSE
                    -- Caso borrar TODOS los pedidos de ese vino entre esas sucursales
                    SELECT NVL(SUM(cantidad),0) INTO v_cantidad_restaurar
                    FROM Solicita
                    WHERE cod_sucursal = p_cod_suc_pide AND cod_sucursal_prov = p_cod_suc_prov 
            ...
            BEGIN
                -- Ahora el INSERT incluye todas las columnas que pide el enunciado
                INSERT INTO Vino (
        ...
        for (DatabaseManager.Delegacion d : DatabaseManager.Delegacion.values()) {
            ...
            try (Connection conn = db.getConnection(d);
                ...
                for (int i = 0; i < procedures.length; i++) {
                    try {
                        // Importante: No usar executeUpdate para CREATE PROCEDURE, usar execute
                        stmt.execute(procedures[i]);
                    } catch (SQLException e) {
                        System.err.println("Error en procedimiento " + (i+1) + ": " + e.getMessage());
                    }
                }
